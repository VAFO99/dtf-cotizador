...
export default function ClientApp() {
  ...
  const selectedVariants = useMemo(() => variantsByColor.flatMap(g => g.items.map(i => ({ color: g.color, talla: i.talla, qty: i.qty }))), [variantsByColor]);
  const totalQty = selectedVariants.reduce((s, i) => s + i.qty, 0);
  ...
  const handleRemoveColorRow = useCallback((c) => setColorRows(prev => prev.filter(x => x !== c)), []);

  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const gid = `${prendaId || "prenda"}-${quoteNum}`;
      const lines = variantsByColor.map(g => ({ qty: g.total, prendaLabel: prenda?.name ?? "Prenda", color: g.color, cfgLabel: selectedPos.join(" + "), tallasSummary: g.items.map(i => `${i.talla}:${i.qty}`).join(", "), groupId: gid, groupLabel: prenda?.name ?? "Prenda", variants: g.items.map(i => ({ sku: buildVariantSku({ prendaLabel: prenda?.name ?? "Prenda", color: g.color, talla: i.talla }), color: g.color, talla: i.talla, qty: i.qty })), sellPrice: 0, lineTotal: 0 }));
  ...
  const S = {
    page: { minHeight: "100vh", background: "#F5F5F7", color: "#1D1D1F", fontFamily: "'Outfit',sans-serif" },
    wrap: { maxWidth: 560, margin: "0 auto", padding: "0 20px" },
    card: { background: "#fff", borderRadius: 20, padding: "28px 24px", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,.04)" },
    lbl: { fontSize: 11, fontWeight: 600, color: "#86868B", marginBottom: 8, textTransform: "uppercase", letterSpacing: ".08em" },
    inp: { width: "100%", background: "#F5F5F7", border: "1.5px solid #E8E8ED", borderRadius: 12, padding: "13px 16px", fontSize: 15, color: "#1D1D1F", fontFamily: "'Outfit'", outline: "none", transition: "border .2s" },
    btn: { background: "#1D1D1F", border: "none", borderRadius: 14, padding: "16px 32px", fontSize: 15, fontWeight: 700, color: "#fff", cursor: "pointer", width: "100%", fontFamily: "'Outfit'", transition: "opacity .2s" },
    btnOut: { background: "transparent", border: "1.5px solid #D2D2D7", borderRadius: 14, padding: "15px 22px", fontSize: 14, fontWeight: 600, color: "#6E6E73", cursor: "pointer", fontFamily: "'Outfit'" },
    chip: (on) => ({ display: "inline-flex", alignItems: "center", padding: "10px 20px", borderRadius: 28, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "1.5px solid", transition: "all .2s", userSelect: "none",
      background: on ? "#1D1D1F" : "#fff", borderColor: on ? "#1D1D1F" : "#D2D2D7", color: on ? "#fff" : "#6E6E73" }),
  };
  ...
  if (loading) return (
    <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} .fade-up{animation:fadeUp .4s ease both}`}</style>
    </div>
  );
  ...
  if (submitted) return (
    <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ maxWidth: 400, width: "100%", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-.03em", marginBottom: 10 }}>Solicitud enviada</h1>
        <p style={{ color: "#6E6E73", fontSize: 15, lineHeight: 1.7, marginBottom: 28 }}>
          Tu solicitud <b style={{ color: "#1D1D1F", fontFamily: "'JetBrains Mono'" }}>#{quoteNum}</b> fue recibida. {deliveryPref === "whatsapp" ? "Te contactaremos por WhatsApp." : deliveryPref === "email" ? "Te enviaremos la cotización por correo." : "Te notificaremos cuando esté lista."}
        </p>
        <div style={{ ...S.card, textAlign: "left", padding: "20px 22px" }}>
          {[["Referencia", `#${quoteNum}`], ["Prenda", prenda?.name ?? "?"], ["Variantes", colorResumen || "—"], ["Posiciones", selectedPos.join(", ")], ["Total", `${totalQty} prendas`], ["Recibir por", deliveryPref === "whatsapp" ? "WhatsApp" : deliveryPref === "email" ? "Correo" : "Descarga"]].map(([k,v]) => (
        ...
        <button onClick={() => { setSubmitted(false); setStep(0); setNombre(""); setWhatsapp(""); setEmail(""); setPrendaId(""); resetVariantSelection(); setSelectedPos([]); setNotas(""); setDeliveryPref("whatsapp"); }}
          style={S.btn}>Nueva solicitud</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={S.page}>
      ...
      <style>{`
        ...
        .cinp{width:100%;background:#F5F5F7;border:1.5px solid #E8E8ED;border-radius:12px;padding:13px 16px;font-size:15px;color:#1D1D1F;font-family:'Outfit';outline:none;transition:border .2s}
        .cinp:focus{border-color:#0071E3;background:#fff}
        .cinp::placeholder{color:#AEAEB2}
        .matrix-wrap{overflow:auto;border-radius:16px;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.04)}
        .matrix-table{width:max-content;min-width:100%;border-collapse:separate;border-spacing:0}
        .matrix-table th,.matrix-table td{padding:12px 10px;border-right:1px solid #F5F5F7;border-bottom:1px solid #F5F5F7;text-align:center}
        .matrix-table thead th{position:sticky;top:0;background:#FAFAFA;z-index:2}
        .matrix-table th:first-child,.matrix-table td:first-child{position:sticky;left:0;text-align:left;background:#fff;z-index:1}
        .matrix-table thead th:first-child{z-index:3;background:#FAFAFA}
        .matrix-table th{font-size:10px;font-weight:700;color:#86868B;text-transform:uppercase;letter-spacing:.08em}
        .matrix-table tr:last-child td{border-bottom:none}
        .matrix-table th:last-child,.matrix-table td:last-child{border-right:none}
        .mci{width:60px;background:#F5F5F7;border:1.5px solid #E8E8ED;border-radius:10px;padding:10px 6px;text-align:center;font-size:16px;font-weight:700;font-family:'JetBrains Mono';color:#1D1D1F;outline:none;transition:all .2s}
        .mci:focus{border-color:#0071E3;background:#fff;box-shadow:0 0 0 3px rgba(0,113,227,.08)}
        .mtc{background:#F0F7FF;font-family:'JetBrains Mono';font-weight:700;color:#0071E3}
        @media(max-width:480px){.mci{width:52px;font-size:14px}}
      `}</style>
      ...
      <header style={{ background: "rgba(245,245,247,.8)", backdropFilter: "blur(20px)", borderBottom: "1px solid #E8E8ED", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ ...S.wrap, height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
      ...
      {/* Steps */}
      <div style={{ ...S.wrap, padding: "20px 20px 0" }}>
      ...
      <div style={{ ...S.wrap, paddingBottom: 100 }}>

        {/* Step 0 */}
        {step===0 && (
          <div className="fade-up">
            <div style={{ textAlign: "center", marginBottom: 28 }}>
              <h2 style={{ fontSize: 34, fontWeight: 800, letterSpacing: "-.04em", lineHeight: 1.1, marginBottom: 10 }}>Personalizá{"\n"}tus prendas.</h2>
              <p style={{ fontSize: 15, color: "#6E6E73", lineHeight: 1.6, maxWidth: 380, margin: "0 auto" }}>Completá el formulario y recibí una cotización personalizada. Sin compromiso.</p>
            </div>
            <div style={S.card}>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div><div style={S.lbl}>Nombre completo</div><input className="cinp" placeholder="Ej: María García" value={nombre} onChange={e=>setNombre(e.target.value)} autoFocus/></div>
                <div><div style={S.lbl}>WhatsApp</div><PhoneInput value={whatsapp} onChange={setWhatsapp} placeholder="tu número" style={{ height: 48 }} /><div style={{ fontSize: 11, color: "#AEAEB2", marginTop: 6 }}>Para enviarte la cotización</div></div>
                <div><div style={S.lbl}>Correo <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: "#AEAEB2" }}>— opcional</span></div><input className="cinp" placeholder="tu@correo.com" value={email} onChange={e=>setEmail(e.target.value)} type="email"/></div>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {[["💧","Resistente","+50 lavados"],["🎨","Colores HD","Sin límites"],["⚡","Rápido","24–48h"],["👕","Universal","Cualquier tela"]].map(([ic,t,d]) => (
                <div key={t} style={{ background: "#fff", borderRadius: 16, padding: "16px 14px", boxShadow: "0 1px 3px rgba(0,0,0,.03)" }}>
                  <div style={{ fontSize: 18, marginBottom: 6 }}>{ic}</div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{t}</div>
                  <div style={{ fontSize: 11, color: "#86868B", marginTop: 2 }}>{d}</div>
                </div>
              ))}
            </div>
            <button style={{ ...S.btn, opacity: canStep1 ? 1 : .35, cursor: canStep1 ? "pointer" : "not-allowed" }} disabled={!canStep1} onClick={()=>setStep(1)}>Continuar</button>
          </div>
        )}
        ...
        {step===1 && (
          <div className="fade-up">
            <div style={S.card}>
              <div style={S.lbl}>Tipo de prenda</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {prendas.map(p=><button key={p.id} style={S.chip(prendaId===p.id)} onClick={()=>handleSelectPrenda(p)}>{p.name}</button>)}
              </div>
            </div>
            {prenda && (
              <div style={{ ...S.card }} className="fade-up">
                <div style={S.lbl}>Tallas y colores</div>
                <p style={{ fontSize: 13, color: "#6E6E73", lineHeight: 1.6, marginBottom: 16 }}>Escribí cantidades en cada cruce de color y talla.</p>
                <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
                  ...
                  <button style={S.btnOut} onClick={handleAddCustomColor}>+ Color</button>
                </div>
                    ...
                    <table className="matrix-table">
                      ...
                      <tbody>
                        {matrixColors.map(color => {
                          ...
                          return (
                            <tr key={color}>
                              <td><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, minWidth: 100 }}><span style={{ fontWeight: 600, color: rt > 0 ? "#0071E3" : "#1D1D1F" }}>{color}</span>{!isBase && <button onClick={()=>handleRemoveColorRow(color)} style={{ border: "none", background: "transparent", color: "#AEAEB2", cursor: "pointer", fontSize: 16 }}>×</button>}</div></td>
            ...
            {totalQty > 0 && <div style={S.card} className="fade-up"><div style={S.lbl}>Notas <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: "#AEAEB2" }}>— opcional</span></div><textarea className="cinp" placeholder="Ej: Tengo el arte listo…" value={notas} onChange={e=>setNotas(e.target.value)} rows={3} style={{ resize: "vertical", minHeight: 80 }}/></div>}
            <div style={{ display: "flex", gap: 10 }}><button style={S.btnOut} onClick={()=>setStep(0)}>← Atrás</button><button style={{ ...S.btn, flex: 1, opacity: canStep2?1:.35, cursor: canStep2?"pointer":"not-allowed" }} disabled={!canStep2} onClick={()=>setStep(2)}>Posiciones</button></div>
          </div>
        )}
        ...
        {step===2 && (
          <div className="fade-up">
            <div style={S.card}>
              <div style={{ textAlign: "center", marginBottom: 16 }}><h3 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-.02em", marginBottom: 4 }}>¿Dónde va el estampado?</h3><p style={{ fontSize: 13, color: "#86868B" }}>Tocá las zonas o seleccioná de la lista</p></div>
            ...
            <div style={S.card}>
              <div style={S.lbl}>Posiciones</div>
                  ...
                  return (
                    <div key={p.key}>
                      <div onClick={()=>setSelectedPos(prev=>prev.includes(p.key)?prev.filter(k=>k!==p.key):[...prev,p.key])} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: on?"#F0F7FF":"#FAFAFA", border: `1.5px solid ${on?"#0071E3":"#E8E8ED"}`, borderRadius: 14, cursor: "pointer", transition: "all .2s" }}>
                        <div style={{ width: 22, height: 22, borderRadius: 7, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: on?"#0071E3":"#F5F5F7", border: `1.5px solid ${on?"#0071E3":"#D2D2D7"}`, transition: "all .2s" }}>
            ...
            <div style={{ ...S.card, background: "#FFFBEB" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#92400E", marginBottom: 10 }}>Archivo de diseño</div>
              {[["✅","PNG sin fondo (300+ DPI)"],["✅","Vector AI, PDF, SVG"],["⚠️","JPG alta res (+1500px)"],["❌","Word, PowerPoint"]].map(([ic,t])=><div key={t} style={{ display: "flex", gap: 8, padding: "6px 0", fontSize: 13, color: "#78716C" }}><span>{ic}</span><span>{t}</span></div>)}
            </div>
            <div style={{ display: "flex", gap: 10 }}><button style={S.btnOut} onClick={()=>setStep(1)}>← Atrás</button><button style={{ ...S.btn, flex: 1, opacity: canStep3?1:.35 }} disabled={!canStep3} onClick={()=>setStep(3)}>Revisar</button></div>
          </div>
        )}
        ...
        {step===3 && (
          <div className="fade-up">
            <div style={{ textAlign: "center", marginBottom: 20 }}><h3 style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-.02em", marginBottom: 4 }}>Revisá tu solicitud</h3></div>
            <div style={S.card}>
              {[["Nombre",nombre],["WhatsApp",whatsapp||"—"],["Prenda",prenda?.name??"?"],["Variantes",colorResumen||"—"],["Posiciones",selectedPos.join(", ")],["Total",`${totalQty} prendas`],...(notas?[["Notas",notas]]:[])].map(([k,v])=>(
            ...
            <div style={S.card}>
              ...
              {[["whatsapp","📱","WhatsApp","Directo a tu chat"],["email","📧","Correo",email?`A ${email}`:"Ingresá tu correo"],["download","📄","Descargar","La podrás ver cuando esté lista"]].map(([val,ic,lb,desc])=>(
                <div key={val} onClick={()=>(val!=="email"||email)&&setDeliveryPref(val)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: deliveryPref===val?"#F0F7FF":"#FAFAFA", border: `1.5px solid ${deliveryPref===val?"#0071E3":"#E8E8ED"}`, borderRadius: 12, cursor: val==="email"&&!email?"not-allowed":"pointer", marginBottom: 8, opacity: val==="email"&&!email?.4:1, transition: "all .2s" }}>
                  <div style={{ width: 20, height: 20, borderRadius: 10, border: `2px solid ${deliveryPref===val?"#0071E3":"#D2D2D7"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {deliveryPref===val && <div style={{ width: 10, height: 10, borderRadius: 5, background: "#0071E3" }}/>}
                  </div>
                  <div><div style={{ fontWeight: 600, fontSize: 13 }}>{ic} {lb}</div><div style={{ fontSize: 11, color: "#AEAEB2", marginTop: 1 }}>{desc}</div></div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10 }}><button style={S.btnOut} onClick={()=>setStep(2)}>← Atrás</button><button style={{ ...S.btn, flex: 1 }} disabled={submitting} onClick={handleSubmit}>{submitting?"Enviando…":"Enviar solicitud"}</button></div>
          </div>
        )}
      </div>
    </div>
